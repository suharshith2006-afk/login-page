...
function onPwdInput(val){
    ...
    // re-check confirm match if filled
...
function onConfirmInput(val){
    ...
    if(val===pwd){
        ...
        document.getElementById('su-confirm').classList.remove('error-state');
        document.getElementById('su-confirm').classList.add('ok-state');
    } else {
        note.style.color='#ef4444'; note.textContent='✗ Passwords do not match';
        document.getElementById('su-confirm').classList.remove('ok-state');
        document.getElementById('su-confirm').classList.add('error-state');
    }
}

/* ── IN-MEMORY USER STORE ── */
/* In production this would be PHP+MySQL — for Task 3 you'll replace this */
const users = [];
...
function doSignIn(){
    ...
    setTimeout(()=>{
        ...
        if(user){
            showSuccess(`Welcome back, ${user.name}!`, 'Redirecting to the portfolio…');
        } else {
...
function doSignUp(){
    ...
    errEl.style.color='#ef4444';
    if(!name||!email||!pwd||!confirm){ errEl.textContent='All fields are required.'; return; }
    if(!emailRx.test(email)){ errEl.textContent='Enter a valid email address.'; return; }
    ...
    setTimeout(()=>{
        ...
        // Prefill sign-in form and switch tab
        ...
        siErr.textContent='Account created! Please sign in with your new credentials.';
    }, 900);
}
...
function showSuccess(title, sub){
    ...
    setTimeout(()=>{ window.location.href='https://my-portfolio-3110.vercel.app/'; }, 2700);
}
